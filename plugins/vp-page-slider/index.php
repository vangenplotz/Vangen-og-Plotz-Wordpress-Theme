<?php
/*
Plugin Name: VP Page Slider
Plugin URI: 
Description: Based on WP Orbit Slider
Version: 1.0
Author: Virtual Pudding (changes by Vangen & Plotz)
Author URI: http://virtualpudding.com / www.vangenplotz.no
License: GPL3
...
// Start Main Class
if( !class_exists('vp_orbit_slider') ){
	
	class vp_orbit_slider{
		...
		function init(){
			...
			add_shortcode( 'vp-orbit-slider', array($this, 'show_page_slider') );
			add_shortcode( 'vp-orbit-related-projects-slider', array($this, 'show_related_projects_slider') );
			  ...
			  ob_start(); ?>
				  <div class="custom-slider<?php if ($category == true) { echo '-' . $category; } ?>"> 
					  <div id="orbit-inside">
						  ...
						  // If slide has target
						  if( $link == TRUE && !empty($post->post_content) ) {
						  echo '<div class="content" style=""' . $datathumb . '>';
						  echo '<div class="slide-content slide-content-' . $imgid . '">' . get_the_content() . '</div>';
						  echo '<a target="' . $target . '" href="' . $link . '"  class="slide-link"' . $datacaption . $datathumb . '><img class="orbit-slide" src="' . $urlimg . '" /></a>';
						  echo '</div>';
		...
		function show_page_slider( $atts ){
			global $post;
			// Get plugin options
			$options = $this->get_options();
			// Get combined and filtered attribute list
			...
			$slider_image_ids = simple_fields_get_post_group_values(get_the_id(), 1, false, 2);

			// If there are results, build slider.  Otherwise, don't show anything.
			if( count( $slider_image_ids ) > 0 ) { 
			  ...
			  ob_start(); ?>
				  <div class="custom-slider<?php if ($category == true) { echo '-' . $category; } ?>"> 
					  <div id="orbit-inside">
						  ...
						  <?php foreach( $slider_image_ids as $slider_image_id ): setup_postdata($post);
						  $img = wp_get_attachment_image_src($slider_image_id[1], 'large' );
						  $urlimg = $img['0'];
						  $imgalt = $slider_image_id[4];
						
						  $imgurl = $slider_image_id[3];
						  $imgpost = $slider_image_id[2];
						  
						  // Get the right image
						  // If no image is specified, and a post is specified
						  if( $urlimg == "" && $imgpost > 0) {
							// Get the slide data for the selected post
							$urlimg = simple_fields_get_post_group_values( $imgpost, 1, false, 2);
							// Get first slide
							$urlimg = $urlimg[0];
							// Get the img src
							$urlimg = wp_get_attachment_image_src($urlimg[1], 'large' );
							$urlimg = $urlimg[0];
						  }
						
						  // Get the right url
						  if( $imgurl > 0 ) {
							$imgurl = get_permalink( $imgurl );
						  }
						  elseif($imgpost > 0) {
							$imgurl = get_permalink( $imgpost );
						  }
						
						  $imgtarget = $slider_image_id[5];
						  if( $imgtarget == "1" ) {
						  ...
						  $imgcontent = $slider_image_id[6];
						  // If slide has target
						  if( $urlimg != "" && $imgurl !== "" && $imgurl !== "0" && $imgcontent !== "" ) {
						  echo '<div class="content" style="">';
						  echo '<a ' . $target . 'href="' . $imgurl . '" class="slide-link">';
						  echo '<div class="slide-content slide-content-main slide-content-' . $slider_image_id[1] . '"><div class="slide-content-text">' . $imgcontent . '</div>';
						  echo '</div>';
						  echo '<img class="orbit-slide" src="' . $urlimg . '" /></a>';
						  echo '</div>';
						  // If not and has html content
						  } elseif( $urlimg != "" && $imgcontent !== "" ) {
						  echo '<div class="content">';
						  echo '<div class="slide-content slide-content-' . $imgid . '">' . $imgcontent . '</div>';
						  echo '<img class="orbit-slide" src="' . $urlimg . '" alt="' . $imgalt . '" />';
						  echo '</div>';
						  } elseif( $urlimg != "" && $imgurl !== "" && $imgurl !== "0" ) { 
						  echo '<a ' . $target . 'href="' . $imgurl . '" class="slide-link"><img class="orbit-slide" src="' . $urlimg . '" /></a>';
						  // Otherwise, lets just use images
						  } elseif( $urlimg != "") {
						  echo '<img class="orbit-slide" src="' . $urlimg . '" alt="' . $imgalt . '" />';
						  }
		...
		function show_related_projects_slider( $atts ){
			global $post;
			$orig_post = $post;
			$vp_taxonomy = 'prosjektkategori'; // Project category used to show slider etc.
			// Get plugin options
			$options = $this->get_options();
			// Get combined and filtered attribute list
			$options = shortcode_atts(array('post_type' => $this->post_type, 
			...
			$categories = get_the_terms( $post->ID, $vp_taxonomy);
			if( $categories ) {
				foreach( $categories as $individual_category ) $category_ids[] = $individual_category->term_id;
				$section_title = 'Lignende prosjekter';
			}
			else {
				// If post does not contain categories, see if children contain categories
				$args = array(
					'numberposts' => 4, // Number of posts to get categories from
				...
				if( $post_children ) {
					foreach( $post_children as $post_child ) {
						$post_child_categories = get_the_terms( $post_child->ID, $vp_taxonomy);
						if( $post_child_categories ) {
							foreach( $post_child_categories as $individual_post_child_category ) {
								$cat_id_already_added = false;
								// Check if cat has already been added to array
								if( isset( $post_child_category_ids ) && count($post_child_category_ids) > 0 ) {
									foreach( $post_child_category_ids as $post_child_category_id ) {
										if( $post_child_category_id === $individual_post_child_category->term_id ) {
											$cat_id_already_added = true;
										}
									}
								}
								// If cat has not been added; add it
								if( !$cat_id_already_added ) {
									$post_child_category_ids[] = $individual_post_child_category->term_id;
								}
							}
						}
					}
					if( count( $post_child_category_ids ) > 0 ) {
						$category_ids = $post_child_category_ids;
					}
				}
				$section_title = 'Prosjekteksempler';
			}
			...
			if( count( $category_ids ) > 0 ) {
 			...
 			// If there are categories, build slider.  Otherwise, don't show anything.
			if( count( $category_ids ) > 0 && $my_query->have_posts() ) {
              // Begin Output
			  ob_start(); ?>
				<header>
					<h1 class="entry-title"><?php echo( $section_title ); ?></h1>
				</header>
				  <div class="custom-slider<?php if ($category == true) { echo '-' . $category; } ?> two-column-wrapper"> 
					  <div id="orbit-inside-projects">
	 
						  <?php
						$i = 1;
						 while( $my_query->have_posts() ): setup_postdata($post);
						  $my_query->the_post();
						  if(is_odd($i)) {
							echo('<div class="content content-row">');
						  }
							?>
								<div class="content-wrapper<?php if(is_odd($i)){echo(' content-wrapper-left');}else{echo(' content-wrapper-right');} ?>">
									<?php if ( has_post_thumbnail()) : ?>
									<div class="content-cell-thumb"><a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_post_thumbnail('thumbnail'); ?></a></div>
									 <?php endif; ?>
									<div class="content-cell-text">
				...
				<?php
					if ( count( $category_ids ) > 1 ) {
						// If more than one category, lets show it in a list for ease of reading
						echo('<h2>Prosjektkategorier:</h2>');
						echo( '<ul class="category-list" style="margin-top: 1em;">' );
						foreach(  $category_ids as $category_id ) {
							$term = get_term( $category_id, $vp_taxonomy );
							echo( '<li>' );
							if( $term->count > 1 ) {
								// Only link to page if more than one page (this) with category
								echo( '<a href="' . get_term_link( $term->slug, $vp_taxonomy ) . '" title="Se alle ' . $term->count . ' ' . strtolower( $term->name ) . '." class="biglink">' );
							}
							echo( $term->name );
							if( $term->count > 1 ) {
								// Only show term count and link-end if more than one project with category
								echo( ' (' . $term->count . ')</a>' );
							}
							echo( '</li>' );
						}
					...
					elseif ( count( $category_ids ) > 0 ) {
						echo('<h2>Prosjektkategori:</h2><p style="margin-top: 1em;">');
						foreach(  $category_ids as $category_id ) {
							$term = get_term( $category_id, $vp_taxonomy );
							if( $term->count > 1 ) {
								// Only link to page if more than one page (this) with category
